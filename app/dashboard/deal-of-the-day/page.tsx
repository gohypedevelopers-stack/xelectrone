...
export default async function DealOfTheDayPage() {
  ...
  const defaultProduct = products.find((product) => product.slug === defaultDealOfTheDay.productSlug);
  ...
  return <TooltipProvider><SidebarProvider className="min-h-svh"><AppSidebar /><SidebarInset><DealOfTheDayEditor key={deal ? "saved-deal-pricing" : "default-deal-pricing"} deal={editableDeal} products={products.map((product) => ({ id: product.id, name: product.name, slug: product.slug, price: product.price, oldPrice: product.oldPrice, description: product.description, image: product.mainImage }))} /></SidebarInset></SidebarProvider></TooltipProvider>;
}
