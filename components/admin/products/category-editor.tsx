      ...
      <div className="mx-auto max-w-[968px]">
        ...
        <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1fr)_318px]">
          ...
          <aside className="space-y-4">
            <SectionCard title="Status"><div className="px-4 pb-4"><Select value={status} onValueChange={(value) => { if (value) setStatus(value); setSaved(false) }}><SelectTrigger className="w-full rounded-lg border-black/25 !bg-white text-black shadow-none"><SelectValue /></SelectTrigger><SelectContent className="bg-white text-black"><SelectItem value="active">Active</SelectItem><SelectItem value="draft">Draft</SelectItem></SelectContent></Select></div></SectionCard>
            <SectionCard title="Category organization"><div className="space-y-4 px-4 pb-4"><div className="grid gap-1.5 text-sm text-black/75"><span>Parent category</span><Select value={parentId || "root"} onValueChange={(value) => { if (value) setParentId(value === "root" ? "" : value); setSaved(false) }}><SelectTrigger className="w-full rounded-lg border-black/25 !bg-white text-black shadow-none"><SelectValue /></SelectTrigger><SelectContent className="bg-white text-black"><SelectItem value="root">No parent category</SelectItem>{defaultProductCategories.filter((option) => option.id !== category?.id).map((option) => <SelectItem key={option.id} value={option.id}>{option.title}</SelectItem>)}</SelectContent></Select></div><label className="flex items-center justify-between gap-3 rounded-lg border border-black/10 px-3 py-2.5 text-sm"><span><span className="block font-medium">Store visibility</span><span className="mt-0.5 block text-xs text-black/55">Customers can browse this category.</span></span><Switch checked={visible} onCheckedChange={(checked) => { setVisible(checked); setSaved(false) }} aria-label="Toggle store visibility" /></label></div></SectionCard>
