...
function IconButton({
  ...
  return (
    <button
      type="button"
      aria-label={label}
      className="inline-flex size-9 items-center justify-center rounded-full text-[#1d1d1f] transition-opacity duration-200 hover:opacity-70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black/20"
    >
      {children}
    </button>
  );
...
function FlatNavLink({ label, onClick }: { label: string; onClick?: () => void }) {
  return (
    <Link
      href="/"
      onClick={onClick}
      className="inline-flex h-full min-w-max items-center px-3 text-[13px] font-normal tracking-[-0.01em] text-[#1d1d1f] transition-opacity duration-200 hover:opacity-70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black/20"
    >
      {label}
    </Link>
  );
  ...
  return (
    <div className="relative h-full">
      <button
        ...
        onClick={onToggle}
        className="inline-flex h-full min-w-max items-center gap-1 px-3 text-[13px] font-normal tracking-[-0.01em] text-[#1d1d1f] transition-opacity duration-200 hover:opacity-70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black/20"
      >
        <span>{label}</span>
        <ChevronDown
...
export default function Navbar() {
  ...
  return (
    ...
    >
      <div className="mx-auto flex h-[64px] max-w-[1600px] items-center px-4 sm:h-[72px] sm:px-6 lg:px-8">
        <Link
          href="/"
          aria-label="XElectron home"
          className="inline-flex shrink-0 items-center justify-center rounded-full text-[#1d1d1f] transition-opacity duration-200 hover:opacity-70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black/20"
        ...
        <nav
          aria-label="Primary"
          className="mx-auto hidden h-full flex-1 max-w-[980px] items-stretch justify-between overflow-visible whitespace-nowrap px-6 lg:flex lg:gap-2"
        >
        ...
        <div className="ml-auto hidden shrink-0 items-center gap-1 sm:flex sm:gap-2">
          <IconButton label="Search">
            <Search className="size-4 stroke-[1.7]" />
          </IconButton>
          <IconButton label="Shopping bag">
            <ShoppingBag className="size-4 stroke-[1.7]" />
          </IconButton>
        </div>
        ...
        <div className="ml-auto flex items-center gap-1 sm:hidden">
          ...
          <button
            ...
            className="inline-flex size-9 items-center justify-center rounded-full text-[#1d1d1f] transition-opacity duration-200 hover:opacity-70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black/20"
          >
            {mobileOpen ? <X className="size-4 stroke-[1.8]" /> : <Menu className="size-4 stroke-[1.8]" />}
          </button>
        </div>
      </div>
